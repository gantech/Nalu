...
ContinuitySinSrcElemKernel<AlgTraits>::ContinuitySinSrcElemKernel(
...
template<typename AlgTraits>
void
ContinuitySinSrcElemKernel<AlgTraits>::setup(const TimeIntegrator& timeIntegrator)
{
    ...
    template<typename AlgTraits>
void
ContinuitySinSrcElemKernel<AlgTraits>::execute(
  ...
  ScratchViews<DoubleType>& scratchViews)
{
  ...
  for ( int ip = 0; ip < AlgTraits::numScvIp_; ++ip ) {
    ...
    rhs(nearestNode) += u_infty_ * stk::math::cos(omega_ * cur_time_ - xCoord) * v_scv_volume(ip);
    
  }
}

INSTANTIATE_KERNEL(ContinuitySinSrcElemKernel);
